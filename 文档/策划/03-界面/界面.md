...
**入口：** 市政厅 Actions 主页左侧「牛马」按钮（`TownHallActionsBeastButton` 在 `ScreenEvent.Init.Post` 注入，雇佣兵正下方）。模块 `isPageVisible()=false`，不走 MC 模块侧栏（1214+ 市政厅侧栏被关掉）。页内「返回」回到 Actions。
