...
冷启动第一座建造者是例外：先放小屋方块（不要求已有建造者），牛马雇一名无业市民进这座小屋，再下建造工单。

**不做：** 自己当建造者填方块；自动点研究；按 P0–P4 阶段推进。完整发展路径以固定式为准。
...
| 牛马正在生成/配送（没有空闲牛马） | `beast_busy` |
| 冷启动且第一座建造者尚未雇到人 | `waiting_for_builder` |
...
3. 不铺路。
4. **先放 hut 方块**，此时不下工单（MineColonies 在无人可雇时会拒绝工单并导致本模组回滚方块）。
5. 牛马把无业市民雇进这座小屋（`HiringMode.AUTO`）。
6. 雇到人后再 `requestUpgrade`，工单认领在该小屋自己身上，让新建造者盖自己的工位。
7. 施工期间规划停在 `builder_construction_pending`；若还在等人则 `waiting_for_builder`。
