...
> 技术对照：[../../技术/补给工作循环.md](../../技术/补给工作循环.md)  
> 数值：[../02-数值/数值.md](../02-数值/数值.md)
...
- 走到 `getDeliveryPosition`（市民位置或建筑位置；子请求经父链/requester 建筑解析）。
- 先履约请求，成功后再从牛马背包扣物品；在送达范围内失败则重试，超过 `deliveryTimeoutTicks`（默认 200）后强制放置并完成请求。
- 履约优先级：有市民实体 → 市民背包 + overrule → 当前请求 token 直接 overrule → 建筑 open request → 再次 overrule 兜底。
...
队列优先级：配送员请求用其 `getPriority()`；普通 `IDeliverable` 为 0；其他为 `Integer.MIN_VALUE`。取出时再次验证仍卡住，并立即标记 in-flight。

队列与进行中的生成 **不写进存档**。重启或卸载世界后重新扫描；进行中的生成作废。
