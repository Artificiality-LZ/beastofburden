...
农田 **不走建造者工单**：`PlanningInstantBuild.pasteBlueprint` 瞬间粘贴，再注册 `FarmField.create(BlockPos)`（873；新版本回退 `create(BlockPos, Level)`）并 `FarmerFieldsModule.assignExtension`。

模拟殖民地 1 级农夫最多 **1** 块田（上限 = 小屋等级）。固定式 FIELD 数量超过当前全部农夫可挂田数时，**跳过多余的田**，不为凑田数去升级农夫。
