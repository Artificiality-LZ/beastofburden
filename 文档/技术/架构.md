...
`beastofburden.mixins.json` 的 `mixins` / `client` 均为空，且未通过 `--mixin.config` 注册（ModLauncher 启动过早读不到该文件会崩）。Gradle Mixin 插件仍在。1.1.873 市政厅侧栏原生可见，不要为侧栏再加 mixin，除非升到需要 `shouldRenderDefaultSidebar` 的版本。
