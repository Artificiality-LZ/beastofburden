...
ColonyPlannerDriver（门控）
  → ColonyPlanner.tick → PlanningService.tick
    ...
    → 冷启动：ColdStartManager.bootstrap（雇建造者 + 下工单）
...
市政厅 Actions「牛马」入口以 `TownHallActionsBeastButton`（`ScreenEvent.Init.Post`）为准。`mods.toml` `[[mixins]]` 仍注册 `WindowMainPageMixin`，但 Forge userdev 常常不加载该 config（不要用 Gradle `mixin { config }` / `--mixin.config`，ModLauncher 启动过早读不到资源会崩）。mixin 若生效则把构造参数 `BuildingTownHall.View` 交给同一 helper。**禁止**把 `WindowMainPage` 当成 `AbstractModuleWindow`（1214+ 继承已改，会 `VerifyError`）。不依赖 `shouldRenderDefaultSidebar`。
