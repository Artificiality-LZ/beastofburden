...
市政厅 Actions「牛马」入口以 `TownHallActionsBeastButton`（`ScreenEvent.Init.Post`）为准。`mods.toml` `[[mixins]]` 仍注册 `WindowMainPageMixin`，但 Forge userdev 不会加载该 config（不要用 Gradle `mixin { config }` / `--mixin.config`，ModLauncher 启动过早读不到资源会崩）。mixin 若生效只是早一步调用同一 helper。不依赖 `shouldRenderDefaultSidebar`。
