...
经 `mods.toml` `[[mixins]]` 注册 `beastofburden.mixins.json`（**不要**用 Gradle `mixin { config }` / `--mixin.config`，ModLauncher 启动过早读不到资源会崩）。当前 client mixin：`WindowMainPageMixin`（市政厅 Actions「牛马」入口）。不依赖 `shouldRenderDefaultSidebar`。
