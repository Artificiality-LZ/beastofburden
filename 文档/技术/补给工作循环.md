...
先 `ColonyLogistics.fulfillRequest`，成功后再从牛马背包扣对应堆。失败则在送达范围内重试；超过 `deliveryTimeoutTicks`（默认 200）后 `forceFulfillRequest` 强制放置并完成请求。

`ColonyLogistics` 扫描 `ColonyBuildings.getAllBuildings`（不只市民 work/home）。

`findRequestTargetForDelivery`：openRequests → requester 建筑位置 → 父请求链（子原料请求）。

`fulfillRequest`：

1. 有市民实体 → 背包 + `overruleNextOpenRequestOfCitizenWithStack`（含 crafter assignedTasks 子请求）
2. 对**当前请求 token** 直接 `overruleRequest`
3. 建筑 open request（citizen id **-1**）→ 建筑库存 + `overruleNextOpenRequestWithStack`
4. 再次 direct overrule 兜底
