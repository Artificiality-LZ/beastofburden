...
| `client` | 配置屏、客户端 setup、Actions 牛马按钮 | `ClientSetup`、`BeastofburdenConfigScreen`、`TownHallActionsBeastButton` |
...
| `mixin` | 可选早期注入（与 ScreenEvent helper 同一入口） | `WindowMainPageMixin` |
...
| `beastofburden.mixins.json` | client：`WindowMainPageMixin`（userdev 常不加载；入口以 ScreenEvent 为准） |
