...
- Town Hall Actions «Beast» button is injected by `TownHallActionsBeastButton` (`ScreenEvent.Init.Post`). Optional `WindowMainPageMixin` calls the same helper with the constructor building view. Do not treat `WindowMainPage` as `AbstractModuleWindow` (1214+ hierarchy change causes `VerifyError`). Declare mixins in `mods.toml` `[[mixins]]` instead of `--mixin.config`.
